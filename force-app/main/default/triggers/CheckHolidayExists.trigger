Trigger CheckHolidayExists on Holiday__c (before update,before delete,after undelete) 
{
    If(Trigger.isBefore)
    {
    ...
    Else If(Trigger.isAfter)
    {
        ...
        //loop through all the triggered records
        for(AggregateResult holidayRecord : [
            SELECT Count(Id) countr ,Contact__c c        
            FROM Holiday__c 
            WHERE (Date__c IN : dateSet) GROUP BY Contact__c])
            {
            if(Integer.valueOf(holidayRecord.get('countr'))>1){                   
                holidayMap.get((Id)holidayRecord.get('c')).addError('Holiday Already Exists');
            }
        }
    }
}